...
// prevent opening of data files directly
if (in_array($extension, array ('json'))) {
    return print ('.' . $extension . ' ist nicht erlaubt');
}

// thumbnail size
$max_width = intval(filter_input(INPUT_GET, 'width'));
$max_height = intval(filter_input(INPUT_GET, 'height'));

// thumbnail quality with default
$quality = MIN(MAX(intval(filter_input(INPUT_GET, 'quality')), 0), 100);
if (!$quality) {
    $quality = 75;
}
...
if ($max_width || $max_height) {
    $thumbnailPath = str_replace($extension, ('resized-' . $max_width . 'x' . $max_height . '_' . $quality .  '.' . $extension), $filePath);
}

if (!is_file($filePath)) {
    return print ('Datei wurde nicht gefunden');
}
...
$mime_type = isset($mime_types_map[$extension]) ? $mime_types_map[$extension] : null;
if (!$mime_type) {
    return print ('Mime-Typ nicht gefunden');
}

$filesize = 0;
$thumbnailExists = !empty($thumbnailPath) && is_file($thumbnailPath);
if ($thumbnailExists) {
...
if (!$content) {
    return print ('Datei konnte nicht geladen werden');
}
...
function resizeImage($filename, $max_width, $max_height, $path, $quality = 75)
{
    ...
    $dst_x = 0;
    $dst_y = 0;
    // if width and height is provided, crop image if necessary..
    if ($max_width && $max_height) {
        $width = $max_width;
        $height = $max_height;
        $dst_w = $width;
        $dst_h = $height;
        $resize_percentage_h = $orig_height / $height;
        $resize_percentage_w = $orig_width / $width;
        // portrait images
        if ($resize_percentage_h > $resize_percentage_w)
        {
            $height_aspect_diff = (($max_width / $orig_width) * $orig_height) - $max_height;
            $dst_h += $height_aspect_diff;
            $dst_y -= $height_aspect_diff/2;
        }
        // landscape images
        else if ($resize_percentage_w > $resize_percentage_h) {
            $width_aspect_diff = (($max_height / $orig_height) * $orig_width) - $max_width;
            $dst_w += $width_aspect_diff;
            $dst_x -= $width_aspect_diff/2;
        }
    }
    // max-height limit
